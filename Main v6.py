...
df_test = pd.DataFrame(scaler.transform(df_test.drop(["TotalGHGEmissions"], axis=1)), columns=X.columns)
df_test.head()
...
a = X_train.sort_values(by=['PropertyGFATotal'], ascending=False)
b = X_test.sort_values(by=['PropertyGFATotal'], ascending=False)

print("X_train.head()------", a.head(3))
print("X_test.head()------", b.head(3))
